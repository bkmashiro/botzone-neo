    ...
    while (match.hasRoundsLeft) {
      ...
      const botResults = await Promise.all(
        botEntries.map(async ([botId, request]) => {
          ...
          return [botId, output.response, output.debug ?? null, output.stderr ?? null] as const;
        }),
      );
      ...
      for (const [botId, response, _jsonDebug, stderr] of botResults) {
        // Collect actual process stderr
